...
RUN apt-get -y  locales locales-all

#Creamos la carpeta que almacenará los archivos de textos
RUN mkdir /texto

#Copiamos todos los Scripts
ADD ["OptionsMenu.sh", "statsWords.sh", "statsUsageWords.sh", "findNames.sh", "statSentences.sh", "blankLinesCounter.sh", "caseConverter.sh", "substringReplace.sh", "blockSelection.sh", "palindromeDetection.sh", "./"]
